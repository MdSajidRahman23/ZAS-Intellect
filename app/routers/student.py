...
def _question_payload(question: VivaQuestion | None):
    ...
    level = getattr(question, "difficulty_level", 2) or 2
    ...
    return {
        ...
        "adaptive_note": getattr(question, "adaptive_note", "") or difficulty_note(level),
    }
...
def _existing_raw_scores(session: VivaSession) -> list[float]:
    return [
        float(q.raw_score or q.answer_score or 0)
        for q in sorted(session.questions, key=lambda item: item.q_order)
        if (q.answer or "").strip()
    ]


def _safe_unlink(path: Path, retries: int = 5, delay_seconds: float = 0.15) -> bool:
    """Delete a file safely on Windows."""
...
def _local_fallback_question(session: VivaSession, q_order: int, difficulty_level: int) -> VivaQuestion:
    ...
    question_bank = {
        1: [
            (
                "Concept",
                "Explain the main problem your submission is solving in simple words.",
                "problem, target user, why it matters, simple solution idea",
            ),
            (
                "Workflow",
                "List the basic steps from assignment upload to teacher review.",
                "upload, read file, ask viva, check answer, show teacher result",
            ),
            (
                "Validation",
                "How can a teacher check whether the generated result is fair?",
                "teacher review, transcript, score, evidence, follow-up viva",
            ),
        ],
        2: [
            (
                "Concept",
                "Explain the core problem your submission is solving and why student understanding is important.",
                "AI/copying problem, ownership, understanding verification, teacher workload",
            ),
            (
                "Workflow",
                "Walk through the full workflow step by step from file submission to ZAS-Score.",
                "submission, parsing, question generation, secure viva, evaluation, scoring, dashboard",
            ),
            (
                "Implementation Decision",
                "Which technical decision is most important in your system, and what would fail without it?",
                "secure start, scoring formula, proctoring, database, fallback AI",
            ),
        ],
        3: [
            (
                "Implementation Decision",
                "Defend one key architecture decision and explain its trade-off in a real DIU deployment.",
                "trade-off, reliability, privacy, scalability, teacher workflow",
            ),
            (
                "Validation",
                "Design a validation plan to compare ZAS-Score with teacher judgement and reduce false flags.",
                "test cases, teacher rubric, metrics, false positives, review process",
            ),
            (
                "Limitation/Improvement",
                "Identify one limitation of your prototype and propose a production-ready improvement.",
                "limitation, improvement, deployment, privacy, monitoring, accuracy",
            ),
        ],
    }
...
async def _save_upload_stream(file: UploadFile, destination: Path, max_bytes: int) -> int:
    """Save uploaded file safely and avoid Windows file-lock crash."""
...
def dashboard(
    ...
    sessions = (
        ...
        .filter(VivaSession.student_id == user.id)
...
async def submit_assignment(
    ...
    if settings.ai_privacy_notice and not consent_ai_processing:
        return templates.TemplateResponse(
            request,
            "assignment.html",
            _assignment_context(request, user, "Please accept the AI processing notice before starting the viva."),
            status_code=400,
        )
    ...
    if not file.filename:
        return templates.TemplateResponse(
            request,
            "assignment.html",
            _assignment_context(request, user, "Please choose a submission file."),
            status_code=400,
        )
    ...
    try:
        ...
        return templates.TemplateResponse(
            request,
            "assignment.html",
            _assignment_context(request, user, str(exc)),
            status_code=413,
        )
    ...
    if actual_size == 0:
        ...
        return templates.TemplateResponse(
            request,
            "assignment.html",
            _assignment_context(request, user, "Uploaded file is empty."),
            status_code=400,
        )
    ...
    if settings.enable_adaptive_viva:
        first_question = _create_adaptive_question(
            db,
            viva,
            q_order=1,
            difficulty_level=settings.adaptive_start_difficulty,
        )
        ...
        for index, draft in enumerate(
            ai_engine.generate_questions(extracted_text, count=settings.adaptive_question_count),
            start=1,
        ):
    ...
    audit_log(
        ...
        user.id,
        "submission_uploaded",
        f"submission_id={submission.id}; viva_id={viva.id}; provider={provider_used}",
    )
...
def answer_question(
    ...
    if not secure_has_started(session):
        ...
        return _answer_error(
            ...
            session.id,
            "secure_not_started",
            "Secure viva has not started yet. Camera, microphone, recording, and full-screen checks must pass before answering.",
            status_code=409,
        )
    ...
    if not question or question.session_id != session.id or not active or question.id != active.id:
        return _answer_error(
            ...
            session.id,
            "locked",
            "That question is already locked. Continue with the active viva question.",
            status_code=409,
        )
    ...
    if len(answer) < settings.min_answer_chars:
        return _answer_error(
            ...
            session.id,
            "too_short",
            "Your answer is too short. Please explain the logic, steps, and decision clearly.",
            status_code=400,
        )
    ...
    question.feedback = (
        f"{evaluation.feedback} Adaptive marking: raw {evaluation.score:.1f}, "
        f"{level_label} level official score {official_score:.1f}."
    )
    ...
    audit_log(
        ...
        request,
        user.id,
        "viva_answer_submitted",
        f"session_id={session.id}; question_id={question.id}; score={evaluation.score}; provider={evaluation.provider}",
    )
    ...
    if settings.enable_adaptive_viva and not next_question and answered < total:
        ...
        audit_log(
            ...
            f"session_id={session.id}; q_order={next_question.q_order}; difficulty={next_level}; previous_raw_score={evaluation.score}",
        )
            ...
            return JSONResponse(
                {
                    ...
                    "adaptive_message": (
                        f"Previous answer raw score {evaluation.score:.1f}. "
                        f"Next question level: {difficulty_label(next_question.difficulty_level)}."
                    ),
                }
            )
...
def finish_viva(
    ...
    if not secure_has_started(session):
        audit_log(
            ...
            request,
            user.id,
            "finish_blocked_secure_not_started",
            f"session_id={session.id}",
        )
        db.commit()
        return RedirectResponse(f"/student/viva/{session.id}?error=secure_not_started", status_code=303)

    # Manual Finish should complete the viva with only the submitted answers.
    # Unanswered questions are allowed; scoring/reporting will use the submitted session data.
    finalize_session(db, session, reason="manual_finish", allow_unanswered=True)
    ...
    audit_log(
        ...
        user.id,
        "viva_completed",
        f"session_id={session.id}; reason={session.completed_reason}; zas_score={session.zas_score}",
    )
...
def finish_get():
    raise HTTPException(
        status_code=405,
        detail="Viva finish must be submitted securely from the Finish button.",
    )
...
def result_pdf(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=zas_report_session_{session.id}.pdf"
        },
    )